...
import { addLikes, delComment } from "../Store/AppReducer";

export default function Followers(){
 const follow = useSelector(state => state.menu.Followers);
 const dispatch = useDispatch();

 
 return(
  <>
  <div className="d-flex flex-wrap w-100" style={{margin:'60px'}}>
   {follow.map((f,k)=>{
    return(
     <div key={k} class="like-dislike-container">
	<div class="tool-box">
		<button class="btn-close" onClick={()=>dispatch(delComment(f.fullName))}></button>
	</div>
 <p>{f.fullName}</p>		
 <p>{f.email}</p>
	<p class="text-content">{f.comment}</p>	
 	
	<div class="icons-box">
		<div class="icons">
			<label class="btn-label" for="like-checkbox">
				<span class="like-text-content">{f.likes}</span>
				<input class="input-box" id="like-checkbox" type="checkbox" onClick={()=>dispatch(addLikes(f.fullName))}/>
		...
		<div class="icons">
			<label class="btn-label" for="dislike-checkbox">
				...
				<span class="dislike-text-content">{f.dislikes}</span>
			</label>
		</div>
	</div>
</div>
